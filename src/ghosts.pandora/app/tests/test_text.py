...
def test_return_text_fallback(fallback_text_response):
    ...
    assert "Content-Disposition" in response.headers
    assert response.headers["Content-Disposition"] == "inline; filename=document.html"
    assert "<h1>" in response.content.decode("utf-8")  # Ensure heading is included
    assert "<h2>" in response.content.decode("utf-8")  # Ensure subheadings are included
    assert "<p>" in response.content.decode("utf-8")  # Ensure paragraphs are included
