...
from config.config import WEB_CONTENT_MODEL
...
async def content(
    ...
    try:
        response = content_main(query, content_documents, WEB_CONTENT_MODEL)
        logger.debug(f"Web Content response for {username}: {response}")
